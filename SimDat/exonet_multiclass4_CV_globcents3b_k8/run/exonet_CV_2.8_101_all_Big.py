...
def train_model(n_epochs, kepler_data_loader, kepler_val_loader, model, criterion, optimizer, augment='',savefile=None, cont=None):
    ...
    for epoch in range(startepoch,startepoch+n_epochs):
        ...
        ### loop over batches
        val_pred, val_gt, val_loss, num_corr = np.zeros((4,4)), np.zeros(4), 0, 0
...
if path.exists(path.join(foldname,"exonet_multiclass4_CV_globcents3b_k8_dic.pickle")):
    outputvals=pickle.load(open(path.join(foldname,"exonet_multiclass4_CV_globcents3b_k8_dic.pickle"),'rb'))
else:
    outputvals={}
outputvals[savename]=outputdic
pickle.dump(outputvals,open(path.join(foldname,"exonet_multiclass4_CV_globcents3b_k8_dic.pickle"),'wb'))
