...
def train_model(n_epochs, kepler_data_loader, kepler_val_loader, model, criterion, optimizer, augment='',savefile=None, cont=None):
    ...
    if cont is None:
        ...
        epoch_train_loss = [];      epoch_val_loss = []       ;epoch_val_ap=[];     epoch_val_acc=[]
        ...
        epoch_val_ap=cont['ap_val_epoch']                  ;  epoch_val_acc = cont['epoch_val_acc']
    ...
    for epoch in range(startepoch,startepoch+n_epochs):
        ...
        ### loop over batches
        val_pred, val_gt, val_loss, num_corr = np.zeros(4), np.zeros(4), 0, 0
        ...
        for x_val_data, y_val in kepler_val_loader:
            ...
            num_corr += np.sum(output_pred.cpu().numpy().ravel().astype(int)==y_val.cpu().numpy().ravel().astype(int))
            ...
            val_pred=np.hstack((val_pred,output_val.data.cpu().numpy().ravel()))
            val_gt=np.hstack((val_gt,y_val.data.cpu().numpy().ravel()))
        ...
        epoch_val_acc.append(np.sum((val_pred>0.5).astype(int)==val_gt) / len(val_gt))
        
        ### calculate average precision for this epoch
        #val_gt=np.hstack(val_gt)
        #one_hot_labels=make_one_hot(val_gt,4)
        ...
        epoch_val_ap.append(average_precision_score(val_gt, val_pred, average=None))
        ...
        if epoch_val_loss[-1]<minloss:
            ...
            torch.save(model.state_dict(),path.join(foldname,savename+'_best.pth'))
        ...
        if epoch%5==1:
            ...
            print(epoch,"{0:.3f}".format((tnow-t0)/epoch-startepoch),"s/epoch , LOSS: train  =  ",str(epoch_train_loss[-1])[:6],
                  ...
                  ' ,  acc = '+str(epoch_val_acc[-1])[:6],'='+str(num_corr)+'/'+str(len(kepler_val_loader.dataset)),
            ...
            '''
            dic['ap_val_epoch']=epoch_val_ap
            dic['acc_val_epoch']=epoch_val_acc
...
savename    = "exonet_CV_0.8_101_all_Big"
...
if path.exists(path.join(foldname,"exonet_binary_CV_globcents3b_k8_2_dic.pickle")):
    outputvals=pickle.load(open(path.join(foldname,"exonet_binary_CV_globcents3b_k8_2_dic.pickle"),'rb'))
else:
    outputvals={}
outputvals[savename]=outputdic
pickle.dump(outputvals,open(path.join(foldname,"exonet_binary_CV_globcents3b_k8_2_dic.pickle"),'wb'))
