...
def read_tess_light_curve(filenames, light_curve_extension="LIGHTCURVE", invert=False,use_dv=False,tce=None):
    ...
    if use_dv:
        ...
        try:
            with fits.open(dv_fname) as hdu_list:
                ...
                test_epcs=[]
                nrst_epcs=[]
                for n in range(1,len(hdu_list)-1):
                    if 'TEPOCH' in hdu_list[1].header:
                        #Finds nearest epoch and period
                        dvepochs+=[hdu_list[n].header['TEPOCH']]
                        dvperiods+=[hdu_list[n].header['TPERIOD']]
                dvepochs=np.array(dvepochs)
                dvperiods=np.array(dvperiods)
                if len(dvepochs)==0:
                    nodv=True
                else:
                    nodv=False
                    test_epcs=np.arange(tce['TEPOCH']-50*tce['TPERIOD'], tce['TEPOCH']+50*tce['TPERIOD'], tce['TPERIOD'])
                    nrst_epcs=np.array([test_epcs[np.argmin(abs(test_epcs-hdu_list[n].header['TEPOCH']))] for n in range(1,len(hdu_list)-1)])
                    #print(len(dvepochs),len(dvperiods),len(nrst_epcs))
                    #print((abs(tce['TPERIOD']-dvperiods)/tce['TPERIOD'])**2)
                    #print((abs(nrst_epcs-dvepochs)/tce['TPERIOD'])**2)
                    idx=np.argmin((abs(tce['TPERIOD']-dvperiods)/tce['TPERIOD'])**2+(abs(nrst_epcs-dvepochs)/tce['TPERIOD'])**2)
                    ...
                    tce_colnames=['TPERIOD','TDEPTH','TSNR','TDUR','INDUR','IMPACT',
                                  'DRRATIO','RADRATIO','PRADIUS','MAXMES','MAXSES','NTRANS']
                    dv_data={c:hdu_list[idx+1].header[c] if hdu_list[idx+1].header[c] is not fits.card.UNDEFINED else np.nan for c in tce_colnames }

                    dv_data['TDUR_LOG_RATIO']=np.log10(dv_data['TDUR']/24/(dv_data['TPERIOD']*np.arcsin(1/dv_data['DRRATIO'])/np.pi))
                    dv_data['PRAD_LOG_RATIO']=np.log10(dv_data['PRADIUS']/13)
                    dv_data['SESMES_LOG_RATIO']=np.log10((dv_data['MAXSES']*np.sqrt(dv_data['NTRANS']))/(dv_data['MAXMES']))

                    star_colnames=['RADIUS','LOGG','TEFF','TESSMAG','PMTOTAL','MH']
                    dv_data.update({c:hdu_list[0].header[c] if hdu_list[0].header[c] is not fits.card.UNDEFINED else np.nan for c in star_colnames })
                    dv_data['tce_label']='X'
        except IOError:
            print("DV file corrupted")
            nodv=True
        if not nodv:
        ...
        #Getting TCE data from DV file (because why not)
    ...
    for filename in filenames[1:]:
        try:
            ...
            print("Corruption with ",filename,"?")
    ...
    if use_dv and not nodv and np.sum(np.isnan(dvflux))<0.5*len(dvtime):
        ...
        tce['nanfill']=np.nan
        tce['RADRATIO']=np.sqrt(tce['TDEPTH']/1e6)
        tce['PRADIUS']=tce['rad']*tce['RADRATIO']*110
        tce['NTRANS']=np.clip(np.floor(27.1/tce['TPERIOD']),2,50)
        tce_colnames={'TPERIOD':'TPERIOD','TDEPTH':'TDEPTH','TSNR':'TSNR','TDUR':'TDUR',
                      'INDUR':'nanfill','IMPACT':'nanfill','DRRATIO':'nanfill','RADRATIO':'RADRATIO',
                      'PRADIUS':'PRADIUS','MAXMES':'MAXMES','MAXSES':'MAXSES','NTRANS':'NTRANS'}
        dv_data={c:tce[tce_colnames[c]] for c in tce_colnames}
        dv_data['TDUR_LOG_RATIO']=np.nan#np.log10(tce['TDUR']/24/(tce['TPERIOD']*np.arcsin(1/tce['DRRATIO'])/np.pi))
        dv_data['PRAD_LOG_RATIO']=np.log10(tce['PRADIUS']/13)
        dv_data['SESMES_LOG_RATIO']=np.log10((tce['MAXSES']*np.sqrt(tce['NTRANS']))/(tce['MAXMES']))
        tce['PMTOTAL']=np.sqrt(tce['pmDEC']**2+tce['pmRA']**2)
        star_colnames={'RADIUS':'rad','LOGG':'logg','TEFF':'Teff','TESSMAG':'Tmag','PMTOTAL':'PMTOTAL','MH':'MH'}
        dv_data.update({c:tce[star_colnames[c]] for c in star_colnames})
        dv_data['tce_label']='X'
    elif not use_dv:
        dv_data=None
    ...
    return all_time, all_flux, all_centroid, all_valid_indices, dv_data
...
def CutAnomDiff(flux,thresh=4.5):
        ...
        nanless=~np.isnan(flux)

        diffarr=np.vstack((np.diff(flux[nanless][1:]),np.diff(flux[nanless][:-1])))
        diffarr/=np.nanmedian(abs(diffarr[0,:]))
        anoms=np.hstack((True,((diffarr[0,:]*diffarr[1,:])>0)+(abs(diffarr[0,:])<thresh)+(abs(diffarr[1,:])<thresh),True))*np.isfinite(flux[nanless])
        allanoms=np.zeros(len(flux)).astype(bool)
        allanoms[nanless]=anoms
        return allanoms
...
def MedianBin(time,flux,centroid,normals,t_min,t_max,nbins,viewtype='glob',ndurs=5):
    ...
    view_cen=np.array([np.nanmedian(centroid[bins_i==i]) for i in np.arange(1,nbins+1)])
...
def Process(tess_data_dir, tce_row, simrun='TSOP-301-4-sector-run', out_data_dir='Processed', nglob=1001,nloc=101,ndurs=5,cut_anoms=True,dospline=False,use_dv=False):
    ...
    all_time, all_flux, all_centroid, valid_indices, dv_data = read_tess_light_curve(fnames, use_dv=use_dv, tce=tce_row)
    #spline
    if dospline:
    ...
    pickle.dump(dv_data,open(path.join(out_data_dir,out_fname+'_view_info.pickle'),'wb'))
